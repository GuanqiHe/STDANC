...
struct datapack_t
{
	...
	array_t u;
	array_t phi;
	MSGPACK_DEFINE_MAP(t, y, d, u, phi);
};

int32 CVICALLBACK DoneCallback(TaskHandle taskHandle, int32 status, void *callbackData);

void (*controlTaskRun)(float64 *readData, float64 *writeData) = NULL;
...
int main(void)
{
	...
	const float runTime = 5, warmUp = 1.0; // sec
	...
	for (int i = 0; i < runTime * sampleFs / samplesPerChan; i++)
	{
		...
		for (int i = 0; i < samplesPerChan; i++)
		{
			...
			float64 rate = globalTime - warmUp > 2.37 ? 1.0 : (globalTime - warmUp) / 2.37;
			float64 phi = (rate)*2 * PI * 0.525;

			float64 d = 1.4 * sin(globalTime * dist_freq * 2 * PI);
			float64 u = 1.4 * sin(globalTime * dist_freq * 2 * PI + phi);
			float64 y = (read_origin[i] - 0.00025) * 8000;

			data.t.push_back(globalTime);
			data.d.push_back(d / 1.4 * 2.0);
			data.u.push_back(u / 1.4 * 2.0);
			data.y.push_back(y);
			data.phi.push_back(phi);
