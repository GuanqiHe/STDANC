...
public:
	...
	AFCController(float64 w_star, float64 dt, float64 a, float64 e, float64 bound = 3.95) : w(4), w_star_(w_star),
																							dt_(dt),
																							bound_(bound), alpha(a), epsilon(e)
	{
		tw = 0.0;
		out = 0.0;
		w[0] = 0.0;
	...
	float64 ydiffCalc(const Eigen::Vector2d &zeta_hat)
	{
		return y_d = G.transpose() * zeta_hat - out;
	}

	float64 uaCalc(const Eigen::Vector2d &zeta_hat)
	{
		return u_a = -epsilon * theta_hat.transpose() * zeta_hat;
	}

	Eigen::Vector2d zetaDynamics(const Eigen::Vector2d &zeta_hat)
	{
	...
	{
		x = x.norm() >= bound_ ? x / x.norm() * bound_ : x;
	}

	void equations(const state_type &y, state_type &dy, double _x)
	{
		Eigen::Vector2d w_hat, dot_w_hat, zeta_hat, dot_zeta_hat;
		w_hat << y[0], y[1];
		zeta_hat << y[2], y[3];

		projection(w_hat, bound_);

		float64 u_a = uaCalc(zeta_hat);
		float64 y_diff = ydiffCalc(zeta_hat);
...
int main(void)
{
	...
	float64 runTime = 30, warmUp = 1.0; // sec
	...
	data.y_f.reserve(totalNumSamples);

	int64 index = 0;
	float64 globalTime = 0, lasty = 0;

	float64 a, e;

	std::cin >> a >> e >> runTime;

	AFCController controller(dist_freq * 2 * PI, 1.0 / sampleFs, a, e);
