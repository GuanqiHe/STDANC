...
int main(void)
{
	...
	std::ofstream stream("scanAmp.bin", std::ios::binary);
	...
	for (int i = 0; i < warmUp * sampleFs / samplesPerChan; i++)
	{
		...
		for (int i = 0; i < samplesPerChan; i++)
		{
			...
			float64 d = 1.0 * sin(globalTime * dist_freq * 2 * PI);
			float64 u = 0;
	...
	for (int i = 0; i < runTime * sampleFs / samplesPerChan; i++)
	{
		...
		for (int i = 0; i < samplesPerChan; i++)
		{
			...
			float64 rate = (globalTime - warmUp) / runTime;
			float64 phi = (0.573) * 2 * PI;
			const float64 A = 0.1, B = 1.9;
			float64 amp = (rate)*B + (1.0 - rate) * A;

			float64 d = 1.0 * sin(globalTime * dist_freq * 2 * PI);
			float64 u = 2.0 * sin(globalTime * dist_freq * 2 * PI + phi) * amp;
			float64 y = (read_origin[i] - 0.00025) * 8000;
